    ...
    switch (change) {
        case ('substr'):
            if ($scope.functionCodeOptions.substr[1]) 
                $scope.sample=$scope.sampleOrigin.substr($scope.functionCodeOptions.substr[0],$scope.functionCodeOptions.substr[1]-$scope.functionCodeOptions.substr[0]); 
        ...
        case ('case'):
            switch ($scope.functionCodeOptions.textCaseOption) {
                ...
                case ('capitalize'):
                    var w = $scope.sample.match(/\w/);
                    $scope.sample=$scope.sample.toLowerCase();
                    if ($scope.sample.charCodeAt(w.index)>96 && $scope.sample.charCodeAt(w.index)<123) $scope.sample=$scope.sample.substr(0,w.index)+String.fromCharCode($scope.sample.charCodeAt(w.index)-32)+$scope.sample.substr(w.index+1);
                    $scope.sampleToReplace = $scope.sample;
                break;
                case ('titleize'):
                    var splittedString = $scope.sample.split(" ");
                    $scope.sample = '';
                    for (var i=0;i<splittedString.length;++i) {
                        if (i>0) $scope.sample+=' ';
                        if (splittedString[i].charCodeAt(0)>96 && splittedString[i].charCodeAt(0)<123) 
                            splittedString[i]= String.fromCharCode(splittedString[i].charCodeAt(0)-32)+splittedString[i].substr(1);
        ...
        case ('replace'):
            if ($scope.functionCodeOptions.replaceMap[0]!=='') 
                for (var i=0; i<$scope.functionCodeOptions.replaceMap.length;i+=2) {
                    var replacePattern = new RegExp($scope.functionCodeOptions.replaceMap[i].toString(),"g");
                    $scope.sample = $scope.sampleToReplace.replace(replacePattern,($scope.functionCodeOptions.replaceMap[i+1]?$scope.functionCodeOptions.replaceMap[i+1].toString():''));
                }
        break;
        default:
            console.log("Unhandled modification");
        break;
      }
    };
